'use client';

import { useEffect, useRef, useState } from 'react';
import gsap from 'gsap';
import { ScrollTrigger } from 'gsap/ScrollTrigger';

gsap.registerPlugin(ScrollTrigger);

export default function Home() {
  const containerRef = useRef(null);
  const [portfolio, setPortfolio] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  useEffect(() => {
    fetch('/api/portfolio')
      .then(res => res.json())
      .then(data => {
        setPortfolio(data);
        setLoading(false);
      })
      .catch(err => {
        console.error('Error fetching portfolio:', err);
        setError(err.message);
        setLoading(false);
      });
  }, []);

  useEffect(() => {
    if (!portfolio) return;

    const ctx = gsap.context(() => {
      gsap.from('.hero-badge', { y: 30, opacity: 0, duration: 0.8, ease: 'power3.out' });
      gsap.from('.hero-title', { y: 60, opacity: 0, duration: 1, ease: 'power3.out', delay: 0.2 });
      gsap.from('.hero-subtitle', { y: 40, opacity: 0, duration: 0.8, ease: 'power3.out', delay: 0.4 });
      gsap.from('.hero-cta', { y: 30, opacity: 0, duration: 0.8, ease: 'power3.out', delay: 0.6 });
      gsap.from('.scroll-indicator', { opacity: 0, duration: 1, delay: 1.2 });

      if (portfolio.settings?.animations !== false) {
        gsap.utils.toArray('.project-card').forEach((card: any, i) => {
          gsap.from(card, {
            scrollTrigger: { trigger: card, start: 'top 85%' },
            y: 60, opacity: 0, duration: 0.8, delay: i * 0.1, ease: 'power3.out'
          });
        });

        gsap.utils.toArray('.contact-link').forEach((link: any, i) => {
          gsap.from(link, {
            scrollTrigger: { trigger: '.contact-section', start: 'top 75%' },
            y: 30, opacity: 0, duration: 0.6, delay: i * 0.1, ease: 'power3.out'
          });
        });

        gsap.from('.stat-item', {
          scrollTrigger: { trigger: '.stats-grid', start: 'top 80%' },
          y: 30, opacity: 0, duration: 0.6, stagger: 0.1, ease: 'power3.out'
        });
      }
    }, containerRef);

    return () => ctx.revert();
  }, [portfolio]);

  if (loading) {
    return (
      <div className="loading">
        <div className="loading-spinner"></div>
      </div>
    );
  }

  const theme = portfolio?.theme || {};
  const settings = portfolio?.settings || {};

  return (
    <main ref={containerRef}>
      <div className="bg-pattern"></div>
      <div className="grid-overlay"></div>

      <section className="hero">
        <div className="hero-content">
          <div className="hero-badge">
            <span>Portfolio</span>
          </div>
          
          <h1 className="hero-title">
            <span className="highlight">{portfolio?.profile?.name || 'Mi Portfolio'}</span>
          </h1>
          
          <p className="hero-subtitle">
            {portfolio?.profile?.bio || 'Desarrollador Full Stack creando experiencias digitales'}
          </p>
          
          <div className="hero-cta">
            <a href="#projects" className="btn btn-primary">
              Ver Proyectos
            </a>
            <a href="#contact" className="btn btn-secondary">
              Contacto
            </a>
          </div>
        </div>

        <div className="scroll-indicator">
          <span>Scroll</span>
          <svg width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 14l-7 7m0 0l-7-7m7 7V3" />
          </svg>
        </div>
      </section>

      {portfolio?.profile?.name && portfolio?.profile?.bio && (
        <section className="section">
          <div className="about-grid">
            <div className="about-image">
              {portfolio?.profile?.avatar ? (
                <img src={portfolio.profile.avatar} alt={portfolio.profile.name} className="about-img" />
              ) : (
                <div className="about-img" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '64px' }}>
                  {portfolio.profile.name.charAt(0)}
                </div>
              )}
            </div>
            <div className="about-content">
              <h2>Sobre Mí</h2>
              <p>{portfolio.profile.bio}</p>
              <div className="stats-grid">
                <div className="stat-item">
                  <div className="stat-number">{portfolio.projects?.length || 0}</div>
                  <div className="stat-label">Proyectos</div>
                </div>
                <div className="stat-item">
                  <div className="stat-number">{new Date().getFullYear() - 2020}+</div>
                  <div className="stat-label">Años Exp.</div>
                </div>
                <div className="stat-item">
                  <div className="stat-number">100%</div>
                  <div className="stat-label">Dedicación</div>
                </div>
              </div>
            </div>
          </div>
        </section>
      )}

      <section id="projects" className="section">
        <div className="section-header">
          <div className="section-label">Proyectos</div>
          <h2 className="section-title">Trabajos Destacados</h2>
        </div>

        {portfolio?.projects?.length === 0 ? (
          <div className="empty-state">
            <h3>Próximamente</h3>
            <p>No hay proyectos todavía. Gestiona tu portfolio desde Telegram.</p>
          </div>
        ) : (
          <div className="projects-grid">
            {portfolio?.projects?.map((project) => (
              <a
                key={project.id}
                href={project.githubUrl || project.url}
                target="_blank"
                rel="noopener noreferrer"
                className="project-card"
              >
                {project.imageUrl && (
                  <img src={project.imageUrl} alt={project.name} className="project-image" />
                )}
                <div className="project-content">
                  <h3 className="project-title">{project.name}</h3>
                  <p className="project-description">{project.description}</p>
                  <div className="project-tags">
                    {project.tags?.map((tag) => (
                      <span key={tag} className="tag">{tag}</span>
                    ))}
                  </div>
                </div>
              </a>
            ))}
          </div>
        )}
      </section>

      <section id="contact" className="section contact-section">
        <div className="section-header">
          <div className="section-label">Contacto</div>
          <h2 className="section-title">Conectemos</h2>
        </div>

        <div className="contact-grid">
          {portfolio?.profile?.contact?.email && (
            <a href={`mailto:${portfolio.profile.contact.email}`} className="contact-link">
              <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              Email
            </a>
          )}
          
          {portfolio?.profile?.contact?.github && (
            <a href={`https://github.com/${portfolio.profile.contact.github}`} target="_blank" rel="noopener noreferrer" className="contact-link">
              <svg fill="currentColor" viewBox="0 0 24 24">
                <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
              </svg>
              GitHub
            </a>
          )}
          
          {portfolio?.profile?.contact?.twitter && (
            <a href={`https://twitter.com/${portfolio.profile.contact.twitter}`} target="_blank" rel="noopener noreferrer" className="contact-link">
              <svg fill="currentColor" viewBox="0 0 24 24">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
              </svg>
              Twitter
            </a>
          )}
          
          {portfolio?.profile?.contact?.linkedin && (
            <a href={`https://linkedin.com/in/${portfolio.profile.contact.linkedin}`} target="_blank" rel="noopener noreferrer" className="contact-link">
              <svg fill="currentColor" viewBox="0 0 24 24">
                <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
              </svg>
              LinkedIn
            </a>
          )}
        </div>
      </section>

      <footer className="footer">
        <p>✨ Powered by Kwitt - AI Portfolio OS</p>
      </footer>
    </main>
  );
}
